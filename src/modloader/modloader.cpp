...
int main(int argc, char** argv)
{
    ...
    {
        printf("Your version not supported yet for this loader. Only GOG and "
               "Steams Versions are supported at the moment.\n");
        printf("If you want your version to be supported in the future, please file "
               "an issue with your version and the attached md5 sum provided here:\n");
        printf("md5: ");
        for (size_t i = 0; i < 16; i++)
        {
            printf("%02x", result[i]);
    ...
    HANDLE hThread_injection =
        CreateRemoteThread(processInformation.hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)lpLoadLibraryA, lpRemoteString, 0, NULL);
